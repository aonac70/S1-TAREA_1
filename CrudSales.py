...
from utilities import reset_color,red_color,green_color,yellow_color,blue_color,purple_color,cyan_color,MAGNETA_color,BLANCO_color
...
class CrudSales(ICrud):
    def create(self):
        ...
        gotoxy(30,2);print(green_color+"Registro de Venta")
        gotoxy(17,3);print(green_color+Company.get_business_name())
    ...
    def update(self):
        ...
        gotoxy(2, 1)
        print(cyan_color + "*" * 68 + reset_color)
        gotoxy(2, 2)
        print(cyan_color + "*            ╔════════════════════════════════════════╗            *")
        gotoxy(2, 3)
        print(cyan_color + "*            ║        Actualización de Factura        ║            *")
        gotoxy(2, 4)
        print(cyan_color + "*            ╚════════════════════════════════════════╝            *")
        gotoxy(2, 5)
        print(cyan_color + "*          " +Company.get_business_name()+"         *")
        gotoxy(2, 6)
        print(f"{cyan_color}*                  Ingrese el número de factura:                   *")
        gotoxy(2, 8)
        print(cyan_color + "*" * 68 + reset_color)
        ...
        if not facture:
            gotoxy(35, 8)
            print(f"{green_color}¡La factura no existe!{reset_color}")
            time.sleep(2)
            borrarPantalla()
            return
        facture = facture[0]
        gotoxy(1, 8)
        print(f"{green_color}╔════════════════════════════════════════════════════════════════════════╗")
        print(f"{green_color}║ Factura#:F{facture['factura']} {' '*3} Fecha:{facture['Fecha']} {' '*3} cliente:{facture['cliente']}{' '*(26-len(str(facture['cliente'])))}║{reset_color}")
        
        detalles = facture["detalle"]
        for i, detalle in enumerate(detalles):
            gotoxy(1, 10 + i*6)
            print(f"{BLANCO_color}╔════════════════════════════════════════════════════════════════════════╗{reset_color}")
            print(f"{BLANCO_color}║ Producto: {detalle['poducto']}{' '*(54-len(detalle['poducto']))}       ║{reset_color}")
            print(f"{BLANCO_color}║ Precio: {detalle['precio']}{' '*(57-len(str(detalle['precio'])))}      ║{reset_color}")
            print(f"{BLANCO_color}║ Cantidad: {detalle['cantidad']}{' '*(61-len(str(detalle['cantidad'])))}║{reset_color}")
            print(f"{BLANCO_color}╚════════════════════════════════════════════════════════════════════════╝{reset_color}")
        y_position = 10 + len(detalles)*6 + 3
        print(f"{BLANCO_color}╔════════════════════════════════════════════╗")
        print(f"{BLANCO_color}║   ¿Qué desea actualizar?                   ║")
        print(f"{BLANCO_color}║   1) Agregar más Productos                 ║")
        print(f"{BLANCO_color}║   2) Eliminar productos                    ║")
        print(f"{BLANCO_color}║   Ingrese opción:                          ║")
        print(f"{BLANCO_color}╚════════════════════════════════════════════╝")
        ...
        if opcion == 1:
            ...
            print(red_color + "║" + f"{'ID': <6} {'PRODUCTO': <20} {'STOCK': <10} {'PRECIO': <5}" + " "*(73 - 40) + "║" + reset_color)
            gotoxy(2, y_position + 7)
            print(cyan_color + "╠══════════════════════════════════════════════════════════════════════════════╣" + reset_color)
            for i, producto in enumerate(productos):
                gotoxy(2, y_position + 8 + i)
                print(blue_color + "║" + f"{producto['id']: <6} {producto['descripcion']: <20} {producto['stock']: <10} {producto['precio']: <5}" + " "*(77 - len(str(producto['precio'])) - 40) + "║" + reset_color)
            print(f"{cyan_color}¿Qué producto desea ingresar? \n Ingrese ID del producto: ")
            ...
            if not product:
                ...
                print(f"{blue_color}Producto no existe{reset_color}")
                time.sleep(2)
                borrarPantalla()
                return
            product = product[0]
            ...
            if product_exists:
                ...
                print(f"{BLANCO_color}Actualización de la cantidad de producto")
    ...
    def consult(self):
        ...
        print('\033c', end='')
        line = 1
        gotoxy(2,1);print(BLANCO_color + "█"*90)
