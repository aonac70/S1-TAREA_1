...
from utilities import gotoxy, borrarPantalla, blue_color, red_color, reset_color, green_color
    ...
    def create(self):
        while True:
            ...
            try:
                ...
                gotoxy(3,2);price = valida.solo_decimales("Ingrese el precio del producto: ", "Erro: solo numeros")
                ...
                gotoxy(3,3);print(f"Producto registrado con éxito.")
                time.sleep(2)
        ...
        try:
            while True:
                ...
                while True:
                    ...
                    select = int(validar.solo_numeros("Error: Solo números", 10, len(productos) + 20))
                    if select == 1:
                        borrarPantalla()
                        try:
    ...
    def delete(self):
        ...
        try:
            while True:
                ...
                print(blue_color + "║                    LISTA DE PRODUCTOS                     ║" + reset_color)
                print(blue_color + "╠══════╤═════════════════════════════════════════════════════╣" + reset_color)
                print(blue_color + "║ No. │ DESCRIPCIÓN                     │ PRECIO  │ STOCK   ║" + reset_color)
                print(blue_color + "╟──────┼─────────────────────────────────┼─────────┼─────────╢" + reset_color)
                for i, producto in enumerate(productos):
                    gotoxy(5, 5+i)
                    print(blue_color + f"║ {i+1:<4} │ {producto['descripcion']:<32} │ {producto['precio']:<8} │ {producto['stock']:<7} ║" + reset_color)
                print(blue_color + "╚══════╧═══════════════════════════════════════════════════════╝" + reset_color)
                ...
                if continuar == "si":
                    ...
                    gotoxy(5, len(productos) + 14)
                    print(red_color + "Opción no válida." + reset_color)
                    time.sleep(2)
                    borrarPantalla()
                    continue
